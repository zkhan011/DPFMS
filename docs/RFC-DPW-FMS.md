...
## 16. Offline vector-tile amendment

The implementation now packages a cropped Jebel Ali vector MBTiles archive at `deployment/maps/jebel-ali.mbtiles`. A private Python 3.13.5 tile service performs read-only SQLite access; Tomcat proxies it through same-origin `/api/map/tiles/{z}/{x}/{y}.pbf` and `/api/map/metadata`. The service explicitly converts XYZ Y coordinates to the MBTiles TMS row. The browser uses locally packaged MapLibre GL JS and obtains its bounds, center, zoom range, format and attribution from metadata.

The reproducible refresh pipeline pins `tilemaker/tilemaker:3.0.0`, enforces a maximum requested bounding-box area, downloads a configured Geofabrik OSM PBF only during generation, clips it to the Jebel Ali operating bounds, validates the resulting database and removes the temporary broad extract. CI/release engineering must run the refresh in an internet-enabled environment and review the resulting size and attribution before approving production use.


## 17. Kernel topology persistence amendment

The vector basemap is intentionally not used as an autonomous routing graph. DPW FMS packages a reviewed text JSON plant model with stable geographic properties, 36 points, 70 bidirectional paths, 19 MOVE-capable linked locations and 20 vehicle definitions. In demonstration mode an idempotent servlet listener installs it only into an empty kernel, refreshes routing topology and refuses to replace a user model. The kernel saves its operating model into the persisted `kernel-data` volume on shutdown. Production model synchronization is disabled and remains subject to the standard engineering approval workflow.

The MBTiles SQLite file is stored in source control as Base64 text to support review systems that reject binary patches. Docker and local tooling decode it and enforce the committed SHA-256 before use.
