...
public record FmsConfig(
    boolean mockEnabled, int updateIntervalMs, String provider, boolean offlineEnabled,
    double latitude, double longitude, int zoom, int offlineMinZoom, int offlineMaxZoom,
    String offlineStyleUrl, String googleApiKey
) {
  public static FmsConfig load() {
    ...
    return new FmsConfig(
        bool("FMS_MOCK_DATA_ENABLED", false), integer(
        ...
        integer("FMS_OFFLINE_MIN_ZOOM", 5, 0, 15), integer("FMS_OFFLINE_MAX_ZOOM", 15, 5, 20),
        localPath(env("FMS_OFFLINE_STYLE_URL", "/offline-map/style/style.json")), env(
            "GOOGLE_MAPS_API_KEY", ""
        )
    );
  }
